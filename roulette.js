...
function updateData() {
	...
	R.Items = items
	if (R.autoSave) {
		util.setStorage('roulette_items', items)
	}
	R.dirty = true
}
	...
	function OnRender() {
		...
		if (R.Items && R.Items.length > 0) {
			...
			R.Items.forEach(function (item) {
				...
				drawArc({
					x: R.center.x, y: R.center.y + 15, radius: window.radius,
					rotation: R.rotation + R.lastAngle, angle: item.angle, color: item.color
				}, null, true)
			...
			drawArc({
				x: R.center.x, y: R.center.y + 15, radius: window.radius,
				rotation: R.rotation - 45, angle: 180, color: getColor(0)
			}, '돌려돌려')
			drawArc({
				x: R.center.x, y: R.center.y + 15, radius: window.radius,
				rotation: R.rotation - 45 + 180, angle: 180, color: getColor(1)
			}, '돌림판')
		}
		// pin
		R.buffer.beginPath()
...
$(document).ready(function () {
	...
	R.autoSave = util.getStorage('roulette_autoSave')
	$('#autoSave')[0].checked = R.autoSave
	// 저장된 목록 로드
	var items = util.getStorage('roulette_items')
	...
	$('#autoSave').on('change', e => {
		util.setStorage('roulette_autoSave', e.target.checked)
		R.autoSave = e.target.checked
		if (!R.autoSave) {
			util.setStorage('roulette_items', [])
		}
	})
})
